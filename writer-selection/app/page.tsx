...
export default function WriterSelection() {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        router.push(`/selected-writers?conversationId=${conversationId}`)
        ...
        >
          {writers.map((writer) => (
            <WriterCard
              ...
              isDisabled={isProcessing(writer.writerId) || (selectedWriters.length >= 3 && !selectedWriters.includes(writer.writerId))}
